...
Актуализация по исходникам: приоритет 0 исправлен в коде и прошел автоматические проверки, но живой WebView/reload не проверен. Фазы 1–2 в основном реализованы; пункт 2.6 (`models.json`) требует исправления. Фазы 3–5 не завершены; сквозная проверка фазы 6 и ручные сценарии требуют выполнения. Успешная сборка не подтверждает ручные acceptance criteria.

## Порядок работ

0. Критический баг — исправлен в коде; осталось проверить живой WebView
0.1. Git — аудит diff, коммит текущих изменений, мерж в основную ветку после проверки
0.2. Release/dev — сначала изолированная dev-версия, затем macOS-релиз и безопасная замена stable
1. Фаза 1 — Thinking и streaming в Timeline — реализовано; ручную проверку повторить
2. Фаза 2 — per-session model и reasoning effort — частично; исправить пункт 2.6, повторить ручную проверку
...
1. Проверить весь текущий diff и untracked на секреты/артефакты; подготовить отдельный осмысленный коммит без потери пользовательских изменений. Мерж в основную ветку — после согласования состава коммита.
2. Изолировать dev от stable (bundle identifier, app data, `~/.wii`, plugin storage и WebView storage). До этого не запускать ручной тест поверх stable-данных.
...
4. Затем исправить фазу 2.6 (`models.json` для всех провайдеров), выполнить проверки фаз 1–2. После этого — HOME (фаза 3), external-only plugins (фаза 4), файловый plugin storage (фаза 5), общая проверка (фаза 6).
5. Production release и замена stable — отдельно, после зеленой интеграции и проверки backup/rollback; не смешивать с dev-изоляцией.

---

## Git и macOS release/dev workflow

Статус: **в работе**. Git-переход и dev-изоляция выполняются поэтапно; production-сборку и замену stable не запускать до отдельной проверки.

### Git: переход на основную ветку

- [ ] Определить имя основной ветки и состояние рабочей директории; проверить незакоммиченные и untracked файлы, не потерять пользовательские изменения.
- [ ] Подготовить осмысленный коммит текущего состояния после проверки diff; не включать локальные секреты, временные данные и случайные бинарники.
- [ ] Смержить изменения в основную ветку без потери данных; проверить результат и дальнейшую работу вести в основной ветке. Публикацию на GitHub выполнять отдельно после проверки релиза.

### Отдельная dev-версия на ноутбуке

- [ ] Подготовить воспроизводимые команды/скрипты для dev-запуска и dev-сборки после клонирования: зависимости, необходимые toolchains, сборка bundled `pi`, запуск Tauri; описать команды в `README.md`.
- [ ] Разделить stable/dev через разные Tauri bundle identifier, имя приложения и расположение app data/cache/config/runtime/sessions/provider settings. Проверить также пути к `~/.wii/`, plugin source/config/storage и WebView `localStorage`: изменения dev не должны читать, перезаписывать или удалять данные stable.
- [ ] Обеспечить одновременную установку/запуск stable и dev, отдельные процессы и каталоги; определить отдельные имена артефактов и иконку/маркировку dev для различения версий.
...
- [ ] Подготовить воспроизводимый скрипт production-сборки macOS из чистого клона: pinned зависимости, сборка `pi` под заявленную архитектуру, frontend, Tauri bundle, проверка состава ресурсов.
- [ ] Выпускать готовый macOS-артефакт для GitHub Releases; указать поддерживаемые архитектуры (текущий bundled `pi` — macOS arm64), системные требования, шаги скачивания/установки, checksums и версию.
- [ ] Определить процедуру подписи и notarization для распространяемой сборки macOS; без подписи явно задокументировать ограничения Gatekeeper, не выдавать неподписанный билд за готовый публичный релиз.
- [ ] Подготовить отдельный скрипт «собрать текущий код → заменить установленную stable-версию»: явное подтверждение перед заменой, корректное завершение приложения, backup существующего `.app`, проверка успешной сборки до замены, безопасное восстановление при ошибке.
- [ ] При замене stable сохранять пользовательские app data, сессии, ключи, plugins; не подменять dev-данные и не запускать destructive cleanup. Продумать совместимость форматов и миграции между версиями.
...
Статус: **частично выполнено**. Store/UI/RPC реализованы, но `models.json` генерируется только для активного провайдера; ручные acceptance criteria текущим аудитом не подтверждены.
...
- [ ] Генерировать записи для всех моделей каждого провайдера, а не только выбранной модели. Сейчас `save_providers()` вызывает `write_pi_config(&app, &active)`: `write_pi_config()` перебирает `settings.models` лишь активного провайдера. Учесть ограничения формата pi при совпадающих model ID или разных base URL/API key.
- [ ] Проверить/доработать provider ID, model ID и base URL/headers для моделей **каждого** провайдера в формате pi: текущий `write_pi_config()` создает одну запись `wii-openai` с общими base URL/API key активного провайдера.
