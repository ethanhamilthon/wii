...
3. [ ] Живой ручной прогон закрытия/reopen/двух reload и сверка PID/`.jsonl` **пропущены по просьбе пользователя**; acceptance criteria не подтверждены. Проверены лишь dev `.app` и browser-only smoke.
...
Статус: **исправление внесено; автоматические проверки прошли; ручная интеграция пропущена по просьбе пользователя**.
...
- [ ] Интеграционно проверить число процессов `pi` и файлов истории до/после закрытия вкладок, reload и выхода приложения — ручной сценарий пропущен по просьбе пользователя.
...
- [x] `npm run test` (48 тестов успешно; после изменений повторить в фазе 6).
- [x] `npm run build` (успешно; после изменений повторить в фазе 6).
- [x] Browser-only smoke-test: Session Manager → mock session, Timeline/Composer, 0 page errors. Реальный Tauri WebView проверять отдельно.
...
Статус: **автоматические проверки после dev/models пройдены; ручные сценарии пропущены по просьбе пользователя**. `npm run test` — 48 тестов, `cargo test` — 5 тестов, `npm run build`, `cargo check --manifest-path src-tauri/Cargo.toml`, `git diff --check`, dev/stable unsigned `.app` — успешно. Browser-only smoke: 0 page errors. Интеграция Tauri WebView, GitHub release и замена stable не проверены.
